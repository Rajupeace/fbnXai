...
const adminDB = dbFile('admin', { adminId: process.env.ADMIN_ID || 'admin', password: process.env.ADMIN_PASSWORD || 'admin123' });
...
app.post('/api/admin/login', async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState === 1) {
      ...
      if (!admin) {
        // Create admin in MongoDB if it doesn't exist
        const fileAdmin = adminDB.read() || {};
        admin = new Admin({
          adminId: fileAdmin.adminId || adminId,
          password: fileAdmin.password || password,
          name: 'Administrator'
        });
        await admin.save();
  ...
  try {
    ...
    if ((adminFile.adminId === adminId || adminId === process.env.ADMIN_ID) && (adminFile.password === password || password === process.env.ADMIN_PASSWORD)) {
      ...
      return res.json({
        ok: true,
        token,
        adminData: { adminId: adminFile.adminId || process.env.ADMIN_ID }
      });
    }
