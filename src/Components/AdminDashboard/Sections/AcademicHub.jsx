...
const AcademicHub = ({ courses, students, materials, openModal, handleDeleteCourse, initialSection, onSectionChange, openAiWithPrompt }) => {
    ...
    const renderSyllabusGrid = (year) => {
        ...
        // Filter out override markers only for display, but keep them for logic check
            ...
            if (staticData && staticData.semesters) {
                staticData.semesters.forEach(sem => {
                    sem.subjects.forEach(staticSub => {
                        // Check if subject is already in DB for this specific year/sem/branch
        ...
        let allCourses = merged.filter(c => c.code !== 'EMPTY__OVERRIDE');
    ...
    const renderManagementTable = () => {
        let dbFiltered = courses.filter(c =>
            c.code !== 'EMPTY__OVERRIDE' && // Exclude system marker
            (selectedBranchFilter === 'All' || c.branch === selectedBranchFilter) &&
            (selectedSectionFilter === 'All' || c.section === selectedSectionFilter)
        );

        // Include static subjects in management view as well, so they are searchable
        let merged = [...dbFiltered];
        if (selectedBranchFilter !== 'All') {
            [1, 2, 3, 4].forEach(y => {
                const staticData = getYearData(selectedBranchFilter, String(y));
                if (staticData && staticData.semesters) {
                    staticData.semesters.forEach(sem => {
                        sem.subjects.forEach(staticSub => {
                            ...
                            const isOverridden = courses.some(c =>
                                c.code === 'EMPTY__OVERRIDE' &&
                                (c.name === staticSub.name || c.courseName === staticSub.name || c.name === 'Empty Semester Override') &&
                                String(c.year) === String(y) &&
                                String(c.semester) === String(sem.sem)
                            ...
                            if (!exists && !isOverridden) {
                                merged.push({
                                    ...
                                    section: 'All',
                                    isStatic: true,
                                    id: `static-mg-${staticSub.code}-${y}-${sem.sem}`
                                });
                            }
                        });
                    });
