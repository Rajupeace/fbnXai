...
const StudentLabsSchedule = ({ studentData, enrolledSubjects = [] }) => {
    ...
    const filteredLabs = labSchedule.filter(lab => {
        ...
        return enrolledSubjects.some(sub => {
            const lName = String(lab.labName || '').toLowerCase();
            const sName = String(sub.name || '').toLowerCase();
            const sCode = String(sub.code || '').toLowerCase();
            return lName.includes(sName) || sName.includes(lName) || lName.includes(sCode) || (lab.courseCode && String(lab.courseCode).toLowerCase() === sCode);
        });
    });
