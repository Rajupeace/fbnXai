...
function getAuthHeaders() {
    ...
    if (typeof window !== 'undefined' && window.localStorage) {
        ...
        // Priority: Admin > Faculty > Student (prevents overwriting)
        if (adminToken) {
            headers['Authorization'] = `Bearer ${adminToken}`;
            console.debug('[apiClient] Using admin token');
        } else if (facultyToken) {
            headers['Authorization'] = `Bearer ${facultyToken}`;
            console.debug('[apiClient] Using faculty token');
        } else if (studentToken) {
            headers['Authorization'] = `Bearer ${studentToken}`;
            console.debug('[apiClient] Using student token');
        }
    } else {
...
export async function apiPost(path, body) {
    ...
    if (!adminToken && !facultyToken && !studentToken) {
        console.warn('[apiPost] NO authentication tokens found in localStorage!');
        console.warn('[apiPost] Request to', path, 'will fail - user not logged in');
        console.warn('[apiPost] Available localStorage keys:', Object.keys(localStorage).filter(k => !k.startsWith('persist:')));
    }

    if (path.includes('faculty') && !adminToken) {
        console.error('[apiPost] CRITICAL: Attempting faculty operation without admin token!');
        console.error('[apiPost] AdminToken exists:', !!adminToken);
        console.error('[apiPost] Authorization header:', headers.Authorization ? 'Present' : 'MISSING');
    }
...
export async function apiPut(path, body) {
    ...
    if (path.includes('faculty') && !adminToken) {
        console.error('[apiPut] CRITICAL: Attempting faculty operation without admin token!');
        console.error('[apiPut] Authorization header:', headers.Authorization ? 'Present' : 'MISSING');
    }
...
export async function adminLogin(adminId, password) {
    if (!API_URL) throw new Error('API_URL not configured');
    try {
        console.log('[adminLogin] Attempting login for:', adminId);
        ...
        if (data && data.token) {
            ...
            console.log('[adminLogin] ✅ Admin token saved to localStorage:', data.token.substring(0, 10) + '...');
            console.log('[adminLogin] Token verification - can retrieve:', window.localStorage.getItem('adminToken') ? 'YES' : 'NO');
        } else {
            console.error('[adminLogin] ERROR: Admin login response missing token!', data);
        }

        return data;
    } catch (error) {
        console.error('[adminLogin] Admin login API error:', error);
        throw error;
    }
}
